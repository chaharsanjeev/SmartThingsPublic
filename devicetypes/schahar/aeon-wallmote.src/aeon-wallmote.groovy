...
metadata {
	definition (name: "Aeon WallMote", namespace: "SCHAHAR", author: "Sanjeev Chahar",executeCommandsLocally: true) {
		capability "Actuator"
		capability "Button"
    ...
    preferences {
	    ...
	    input("BATTERY_REFRESH_RATE","enum", title: "Battery Check Refresh?",description: "Check Battery every..", defaultValue: "6 Hour", options: ["1 Hour","2 Hour","3 Hour","4 Hour","5 Hour","6 Hour","7 Hour","8 Hour","9 Hour","10 Hour","11 Hour","12 Hour"], required: true, displayDuringSetup: true)
	...
	tiles (scale: 2) {
        multiAttributeTile(name:"button", type:"generic", width:6, height:4,canChangeIcon: true) {
    		...
    		//	attributeState "default", label:'', backgroundColor:"#28D031", icon: "st.unknown.zwave.remote-controller"
  			//} //End title
            //SECONDARY_CONTROL
            
            tileAttribute ("device.battery", key: "PRIMARY_CONTROL") {
				attributeState "battery", label:'${currentValue} % battery',backgroundColors:[[value: 15, color: "#FF0000"] /*red*/ ,[value: 30, color: "#FF9500"] /*orange*/,[value: 40, color: "#44b621"] /*green*/]
            }//End title
            
        } //end multiattribute
        
        valueTile("battery", "device.battery", decoration: "flat", width: 3, height: 2,canChangeIcon: true) {
			state "battery", label:'${currentValue}%', unit:"", icon : null,backgroundColors:[[value: 15, color: "#FF0000"] /*red*/ ,[value: 30, color: "#FF9500"] /*orange*/,[value: 40, color: "#44b621"] /*green*/]
        }//end standardTile - battery
        ...
        standardTile("configure", "device.needUpdate", inactiveLabel: false, decoration: "flat", width: 6, height: 2) {
        ...
        valueTile("batteryLastChecked", "device.batteryLastChecked", decoration: "flat", width: 6, height: 2) {
			state "batteryLastChecked", label:'Battery Last Checked: ${currentValue}', unit:""
		}//End title
        
		main "battery"
		details(["button", "configure","batteryLastChecked"])
	} //end tile
}

def parse(String description) {

	refreshBatteryLastChecked()
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv1.WakeUpNotification cmd)
{
    ...
    def bettry_ref_rate = Integer.valueOf(BATTERY_REFRESH_RATE.substring(0, BATTERY_REFRESH_RATE.lastIndexOf(" ")).trim())
    
    logging("bettry_ref_rate: $bettry_ref_rate")
    
    if (!state.lastBatteryReport || (now() - state.lastBatteryReport) / 60000 >= 60 * bettry_ref_rate)
    {
        logging("Over "+ (now() - state.lastBatteryReport) / 60000 +"hr since last battery report. Requesting report")
...
def buttonEvent(button, value) {
   ...
   //	refreshBatteryLastChecked() //Do not use this function here, else buttons will stop working
}
...
def refreshBatteryLastChecked(){

	if (state.lastBatteryReport ==null){
       	sendEvent(name: "batteryLastChecked" , value:"??",displayed:false, isStateChange: true)
    }else{
    	//def temp = ((now() - state.lastBatteryReport) / 60000/24).intValue()
        int temp   = (int) (((now() - state.lastBatteryReport) / (1000*60*60))) % 24
       if (temp <=0){
        sendEvent(name: "batteryLastChecked" , value:" Just Now",displayed:false, isStateChange: true)
       }else{
        sendEvent(name: "batteryLastChecked" , value:"$temp Hr.",displayed:false, isStateChange: true)
       }
    }//end if
}//end
